...
async function fork(block_number: number) {
    const alchemy_key = (await readFile(join(__dirname, "..", '.alchemyKey'))).toString().trim()
...
async function run_liquidator(tmp_root: string, liquidator: FlashLiquidator) {
    ...
    await writeFile(private_key_path, accounts[0].privateKey.substr(2))
    ...
    await writeFile(join(tmp_root, "stdout"), stdout)
    await writeFile(join(tmp_root, "stderr"), stderr)
...
describe("flash liquidator", function () {
    ...
    this.beforeEach(async function () {
        tmp_root = await mkdtemp(join(tmpdir(), "flash_liquidator_test"))
    })
